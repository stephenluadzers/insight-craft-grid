...
import { generateSmartWorkflowName, generateExportPackageNames, generateWorkflowDescription, analyzeWorkflowSignature } from "./workflowNaming";
...
export async function generateWorkflowDownloadPackage(
  ...
  const smartName = generateSmartWorkflowName(nodes, { 
    workflowTitle: workflowName,
    includeTimestamp: true 
  });
  const packageNames = generateExportPackageNames(nodes, { workflowTitle: workflowName });
  const description = generateWorkflowDescription(nodes);
  const signature = analyzeWorkflowSignature(nodes);
  
  // Calculate ROI
  const roi = calculateROI(nodes);
  
  // Generate all files with smart names
  const workflowJSON = generateWorkflowJSON(nodes, smartName);
  const reasoningMD = generateReasoningMD(nodes);
  const roiAnalysisMD = generateROIAnalysisMD(roi, smartName);
  const implementationMD = generateImplementationGuideMD(nodes);
  
  // Add files to ZIP with smart naming
  zip.file(`${packageNames.workflowFile}`, workflowJSON);
  zip.file("reasoning.md", reasoningMD);
  zip.file("roi-analysis.md", roiAnalysisMD);
  zip.file("implementation-guide.md", implementationMD);
  
  // Add enhanced README with workflow metadata
  const readme = `# ${smartName}\n\n` +
    `> ${description}\n\n` +
    ...
    `| Type | ${signature.workflowType} |\n` +
    `| Complexity | ${signature.complexity} |\n` +
    `| Node Count | ${signature.nodeCount} |\n` +
    `| AI-Powered | ${signature.hasAI ? 'Yes' : 'No'} |\n` +
    `| Guardrails | ${signature.hasGuardrails ? 'Enabled' : 'None'} |\n` +
    `| Platforms | ${signature.platforms.length > 0 ? signature.platforms.join(', ') : 'None'} |\n\n` +
    `## Package Contents\n\n` +
    `- **${packageNames.workflowFile}** - Executable workflow file (import to n8n, Make, or any compatible platform)\n` +
  ...
  return { blob, fileName: `${packageNames.packageName}.zip` };
}
