...
export async function exportWorkflowComprehensive(
  ...
  try {
    ...
    const n8nFilename = `${smartName}.n8n.json`;
    ...
    zip.file(`${smartName}.n8n.json`, JSON.stringify(n8nWorkflow, null, 2));
    n8nFolder.file(
      "README.md",
      ...
      `3. Select \`${n8nFilename}\` (or \`workflow.json\`) — do not upload the ZIP or the root Remora JSON\n` +
  ...
  for (const platform of ['python', 'typescript', 'docker'] as ExportPlatform[]) {
    try {
      ...
      platformFolder.file(
        ...
        `This complete package focuses on the Remora workflow definition (\`../../${smartName}.json\` and \`../../${smartName}.yaml\`) plus the n8n drop-in (\`../n8n/${smartName}.n8n.json\` or \`../n8n/workflow.json\`).\n`
      );
    } catch (error) {
  ...
  const readme = `# ${smartName} - Complete Export Package\n\n` +
    ...
    `## 📦 Package Contents\n\n` +
    `### Core Files\n` +
    `- \`${smartName}.json\` - Remora workflow definition with embedded credential manifest\n` +
    `- \`${smartName}.n8n.json\` - n8n Import from File workflow JSON\n` +
